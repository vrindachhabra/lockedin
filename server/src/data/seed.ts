...
export const seedAnalytics = {
  ...
  currentStreak: 8,
  weeklyCompletion: 64,
  pendingTasks: seedTasks.filter((task) => !task.completed).length,
  completedTasks: seedTasks.filter((task) => task.completed).length,
  upcomingTests: seedPlacements.filter((placement) => new Date(placement.oaTestDate) >= today).length,
  activeApplications: seedPlacements.filter((placement) => placement.status !== "rejected").length,
  averagePrepProgress: Math.round(
    seedPlacements.reduce((sum, placement) => sum + placement.preparationProgress, 0) / seedPlacements.length
  )
};
